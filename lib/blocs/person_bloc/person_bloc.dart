// lib/blocs/person/person_bloc.dart
  ...
  PersonBloc({required this.repository}) : super(PersonInitial()) {
    ...
    on<AddPerson>((event, emit) async {
      if (state is PersonLoaded) {
        try {
          final created = await repository.add(event.person);
          final updatedList = List<Person>.from((state as PersonLoaded).persons)..add(created);
